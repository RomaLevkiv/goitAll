...
import {statusValues} from '../model/contact.model'
...
// finish controllers
    ...
    async signIn(req, res, next) {
        ...
        //2
            ...
            if(existContact.status === statusValues.NOT_VERIFY) {
                throw new VerificationError("Contact not Verificated")
            }
      ...
      // 5. send response 
        try {
            ...
            const newContact = {
                ...
                avatarURL,
                tokenVerify,
                status: statusValues.NOT_VERIFY
            }            
    ...
    async sendVerificationEmail( tokenVerify, toEmail ) {
        ...
        const verificationLink = `${process.env.SERVER_BASE_URL}/api/auth/verify/${tokenVerify}` 
    ...
    async verifyContact(req, res, next) {
        try {
            ...
            const updContact = await contactModel.updateContactByVerificationToken(tokenVerify)
            if(!updContact) {
                throw new NotFound('Such contact not found!')
            }
            return res.status(200).send("Contact successfully verified!")
        } catch (error) {
            next(error)
        }
    }
